class HashTable(object):
    def __init__(self, length=4):
        # Initiate our array of length 4 with empty values.
    ...
    def get(self, key):
        ...
        # then return its value.
            ...
            for kvp in self.array[index]:
                print("#### self.array[index] " , str(kvp))
                if kvp[0] == key:
                    return kvp[1]
    ...
    """Below methods are to avoid collision"""

    def is_full(self):
        """Determines if the Hashtable is full"""
        items = 0
        # Count how many indexes in our array
        # that is populated with values.
        for item in self.array:
            print("### self.array " , str(item))
            if item is not None:
                items += 1
    ...
    def double(self):
        ...
        for i in range(len(self.array)):
            print("##### range(len(self.array)) " ,str(i))
